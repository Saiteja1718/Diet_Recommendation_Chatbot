import requests
from bs4 import BeautifulSoup
import urllib.parse

Not_found_link='data:image/png;base64,'

def get_images_links(searchTerm):
    """
    Improved image finder with multiple fallback methods
    """
    # Method 1: Try Unsplash (free, high-quality food images)
    try:
        search_query = urllib.parse.quote(f"{searchTerm} food")
        unsplash_url = f"https://source.unsplash.com/400x300/?{search_query}"
        # Return URL directly - Unsplash will provide a random image
        return unsplash_url
    except:
        pass
    
    # Method 2: Try improved Google Images with headers
    try:
        search_query = urllib.parse.quote(searchTerm + " recipe food")
        searchUrl = f"https://www.google.com/search?q={search_query}&site=webhp&tbm=isch"
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
        }
        response = requests.get(searchUrl, headers=headers, timeout=5)
        soup = BeautifulSoup(response.text, 'html.parser')

        img_tags = soup.find_all('img')
        for img in img_tags:
            if 'src' in img.attrs:
                src = img['src']
                if src.startswith("http") and len(src) > 50:  # Filter out tiny images
                    return src
    except:
        pass
    
    # If all methods fail, return placeholder
    return Not_found_link
